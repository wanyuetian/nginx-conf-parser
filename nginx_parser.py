...
def parser(nginx_main_conf, server_conf_path):
    if os.getenv("LOCAL_DEBUG") == "True":
        server_conf_path = "./conf.d/*.conf"
    run_cmd = "ls " + server_conf_path
    ...
    threads = []
    for file in files[:-1]:
        t = threading.Thread(target=nginx_server_info_parser, args=(os.path.join(server_conf_path, file), upstreams, server_infos_queue, lock))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
                ...
                for path in location:
                    if "proxy_pass" in location[path]:
                        for domain in server_info['server']['server_name'].split():
                            ...
                            servers[domain].append([path, proxy_pass, "normal"])
    if os.getenv("LOCAL_DEBUG") == "True":
        nginx_main_conf = "./nginx.conf"
    res = subprocess.run("crossplane lex '%s'" % nginx_main_conf, shell=True, capture_output=True)
    nginx_main_conf = json.loads(res.stdout.decode(encoding="utf8").strip())
    main = nginx_main_parser(nginx_main_conf)
    return main, servers, upstreams
